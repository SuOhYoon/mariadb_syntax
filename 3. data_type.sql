...
alter table author modify age tinyint;
describe author;

-- int : 4바이트(대략, 40억 숫자범위)

-- bigint : 8바이트
-- author, post테이블의 id값 bigint 값 변경
alter table modify column id bigint;

-- decimal(총자릿수, 소수부 자릿수)
...
insert into author(id, email, profile_image) values(7, 'aaa@naver.com', LOAD_FILE('/Users/suohyoon/Downloads/여름수련회 인물사진/이승기.jpeg'))
...
select * from post where title regexpt '[가-힣]'; -- 하나라도 한글이 있으면
...
-- 5월 1일부터 5월 20일까지, 날짜만 입력시 기간부분은 00:00:00이 자동으로 붙음  
...
select * from post cast(where date_format(created_time, '%m') as unsigned) = 5;
