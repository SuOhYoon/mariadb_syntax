-- pk, fk, unique 제약조건 추가 시 해당컬럼에 대해 index페이지 자동생성
...
-- index를 통해 조회성능 향상을 얻으려면, 반드시 where 조건에 해당컬럼에 대한 조건이 있어야함.
select * from author where email="hongildong@nver.com";

-- 만약 where조건에서 2컬럼으로 조회시, 1컬럼에만 index가 있다면
select * from author where name='hong' and email='hongildong@naver.com';

-- 만약 where조건에서 2컬럼으로 조회시, 2컬럼 각각에 index가 있다면
-- 이 경우 db엔진에서 최적의 알고리즘 실헹
select * from author where name='hong' and email='hongildong@naver.com';
...
-- index테스트 시나리오
-- 아래 프로시저를 통해 수십만건의 데이터 insert후에 index 생성 전 후에 따라 조회성능 확인
DELIMITER //
